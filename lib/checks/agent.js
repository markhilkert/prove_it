...
const { SESSION_KEYS, logReview, saveSessionState, loadSessionState, recordReviewerSpend } = require('../session')
...
3. If a verdict-changing finding has NOT been addressed in the code, it
   stands—re-flag it. Note-level findings carry forward as notes: report
   them again, but they MUST NOT produce a failing verdict on their own,
   however many rounds they survive.
...
Do NOT use SKIP because you are unsure—if in doubt, render PASS or FAIL.${notepadInstruction}`
  ...
  // Bill the review before any verdict branching — sync, parallel and async
  // workers all funnel through here, so this is the one place that sees cost.
  if (review.available !== false) {
    recordReviewerSpend(sessionId, {
      task: check.name,
      kind: 'agent',
  ...
  if (review.costUsd != null) extras.costUsd = review.costUsd
  ...
  if (review.finalTurn?.succeeded) {
    ...
    log('TICK', `${check.name} exceeded turn limit — final turn produced verdict (${numTurns}/${maxAgentTurns} turns)`, extras)
  } else if (review.finalTurn && !review.finalTurn.succeeded) {
    const numTurns = review.finalTurn.numTurns != null ? review.finalTurn.numTurns : '?'
    log('BOOM', `${check.name} exceeded turn limit — final turn failed (${numTurns}/${maxAgentTurns} turns)`, extras)
  }

  if (!review.available) {
    const reason = `${review.binary || 'reviewer'} not found`
    log('SKIP', reason, extras)
  ...
  if (review.error) {
    ...
    if (check.model) {
      ...
      log('FAIL', reason, extras)
      return { pass: false, reason, output: '' }
    }
    log('BOOM', review.error, extras)
    return { pass: true, reason: `⚠ ${check.name} crashed: ${review.error}`, output: '', skipped: true }
  }

  if (review.skip) {
    log('SKIP', review.reason, extras)
  ...
  if (review.pass === false) {
    ...
    if (sessionId) {
      ...
      reason += '\n\nBefore acting on this failure, ask yourself: does this apply to the work I\'m currently doing?' +
        '\n- If it MAY apply to your current work—report the findings to the user and' +
        '\n  wait for their direction. Do not start fixing on your own.' +
