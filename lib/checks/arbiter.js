...
const { SESSION_KEYS, saveSessionState, loadSessionState, logReview, recordReviewerSpend } = require('../session')
...
SKIP—suspend the task (benefit of the doubt / unclear situation)`
  ...
  // maxAgentTurns puts the arbiter in JSON mode, which is what reports cost.
  const reviewerCfg = { model: 'haiku', maxAgentTurns: 4 }
...
function handleScriptAppeal (task, result, context) {
  ...
  const costExtra = review.costUsd != null ? { costUsd: review.costUsd } : {}
