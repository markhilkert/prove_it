...
const { readReviewerSpend } = require('./session')
...
/**
 * Cheap reviews round to $0.00 at two decimals, so drop to three below a cent
 * rather than showing a haiku call as free.
 */
function formatCost (costUsd) {
  if (typeof costUsd !== 'number' || !isFinite(costUsd)) return ''
  return costUsd >= 0.01 ? `$${costUsd.toFixed(2)}` : `$${costUsd.toFixed(3)}`
}

/**
 * One-line session total for the monitor header.
 */
function formatSpendSummary (summary) {
  if (!summary || !summary.reviews) return ''
  const reviewLabel = summary.reviews === 1 ? 'review' : 'reviews'
  const unknown = summary.unknownCostCalls
    ? ` (${summary.unknownCostCalls} unknown)`
    : ''
  return `reviewer spend: ${formatCost(summary.totalUsd) || '$0.00'} across ${summary.reviews} ${reviewLabel}${unknown}`
}
...
function formatEntry (entry, maxWidth, opts) {
  ...
  const coloredDur = duration
    ...
    : ''
  const coloredCost = cost
    ? (dim ? `  ${dim}${cost}${reset}` : `  ${cost}`)
    : ''

  let line = `${sessionPrefix}${coloredTime}  ${coloredStatus}  ${reviewer}${coloredDur}${coloredCost}`
...
function formatVerbose (entry) {
  ...
  if (v.prompt != null) {
    ...
    const cost = formatCost(entry.costUsd)
    if (cost) {
      lines.push(`${INDENT}${dim}│${reset} cost: ${cost}`)
    }
    if (v.backchannel) {
...
function printHeader (sessionId, info) {
  ...
  const spend = formatSpendSummary(readReviewerSpend(sessionId))
...
module.exports = { monitor, findLatestSession, listSessions, monitorProject, findProjectLogFiles, projectHash, formatEntry, formatVerbose, formatTime, formatDuration, formatCost, formatSpendSummary, useColor, stripAnsi, visualWidth, normalizeHookTag, middleTruncatePath, truncateReason, progressiveTruncate, watchFile, shouldDisplay, displayStatusOf, extractAppealText, setupQuitKey }
