...
const { isSourceFile, resolveContainedPath } = require('./globs')
...
function makeResolvers (context) {
  ...
  return {
    ...
    session_diff: lazy('session_diff', () => {
      ...
      const tracked = generateDiffsSince(sessionId, projectDir, lastSnapshotId, MAX_DIFF_CHARS)
      // File history tracks whatever the session touched, including files
      // outside the project entirely. Containment is enforced first and always;
      // source scope is applied on top of it, because isSourceFile treats an
      // empty `sources` as unrestricted and so cannot be the project boundary.
      // Anything left unusable falls through to the git fallback below rather
      // than suppressing it.
      const diffs = (tracked || []).filter(d => {
        const rel = resolveContainedPath(d.file, rootDir || projectDir, projectDir)
        if (rel === null) return false
        return isSourceFile(rel, rootDir || projectDir, context.sources)
      })
      if (diffs.length > 0) {
    ...
    signal_message: lazy('signal_message', () => {
      // Prefer the dispatch-local snapshot: the persisted signal may already
      // have been consumed by the launch this prompt is being rendered for.
      if (context._signalSnapshot !== undefined) return context._signalSnapshot?.message || ''
      const { getSignal } = require('./session')
      const signal = getSignal(sessionId)
      return signal?.message || ''
    }),
    phase: lazy('phase', () => {
      // Same reason as signal_message: consuming a `done` resets the persisted
      // phase, and this prompt belongs to the dispatch that consumed it.
