...
// Env gates are pure lookups with no side effects, so they can be consulted
// outside a dispatch. A clause whose env gate cannot pass will never fire, and
// the plan block must not advertise a signal that has no consumer.
function envGatesPass (clause) {
  if (clause.envSet && !process.env[clause.envSet]) return false
  if (clause.envNotSet && process.env[clause.envNotSet]) return false
  return true
}
...
function hasSignalGatedTasks (hooks) {
  ...
  for (const tasks of Object.values(claudeHooks)) {
    for (const task of (tasks || [])) {
      ...
      if (clauses.some(c => c.signal === 'done' && envGatesPass(c))) return true
    }
  }
  return false
}
...
async function dispatch (event, _input) {
  ...
  // TaskCompleted is intentionally inert: completing a task whose subject names
  // the signal command is not the same as executing it. Signal-gated work needs
  // an explicit `prove_it signal <type>` command (intercepted on PreToolUse+Bash).
  // The hook stays registered so Claude settings and `prove_it doctor` stay valid.
  ...
  const context = {
    ...
    maxChars,
    testOutput: '',
    // Snapshot the active signal once per dispatch. Every `when.signal` and
    // every {{signal_message}} in this dispatch reads this value, so clearing
    // the persisted signal mid-loop cannot change what later tasks see.
    _signalSnapshot: getSignal(sessionId) || null,
    // Phase is snapshotted for the same reason: consuming a `done` resets the
    // phase mid-loop, and `when.phase`/{{phase}} are evaluated per task. No
    // reachable path could change the phase mid-dispatch before this, so
    // pinning it keeps task eligibility identical.
    _phaseSnapshot: getPhase(sessionId),
  ...
  // A signal authorizes one Stop cycle, not a standing subscription. The first
  // work that launches *because of* the signal spends it, so a later Stop —
  // including one reached by remediating a blocked Stop — can't relaunch
  // expensive agent work. Remaining tasks in this dispatch keep matching
  // against _signalSnapshot, so one signal still fires a whole batch.
  //
  // Called at each launch site rather than once before them, so that a task
  // which turns out not to launch leaves the authorization armed. Consumption
  // is still on launch and not at settlement: a sibling can block the
  // dispatcher before a launched task's result is ever settled.
  //
  // Both of a signal's lifecycle effects happen here, together. Leaving the
  // `done` phase reset behind in the clean Stop tail would strand the phase
  // permanently: a Stop that blocks after consumption never reaches the tail,
  // and the authorization it would have keyed off is already gone, so no later
  // Stop can perform it either.
  const consumeSignalOnLaunch = () => {
    if (hookEvent !== 'Stop' || signalConsumed || !context._signalAuthorized) return
    signalConsumed = true
    clearSignal(sessionId)
    if (context._signalSnapshot?.type === 'done') setPhase(sessionId, 'unknown')
  }
  ...
  if (hookEvent === 'PreToolUse') {
    ...
    // Claude Code neither shows an approving Stop's reason to the user nor
    // feeds it to the model, so a passing signal-authorized review would
    // otherwise be indistinguishable from one that never ran.
    const announce = hookEvent === 'Stop' && signalConsumed
    protocol.emit(hookEvent, protocol.passDecision(hookEvent),
      `prove_it: ${summary}`, announce ? `prove_it: ${summary}` : undefined)
  }
  ...
  if (hookEvent === 'Stop') {
    ...
    // Unchanged: this still clears an authorization that no task consumed.
    // A consumed one is already gone, along with its phase reset.
...
module.exports = { dispatch, taskMatchesInput, evaluateWhen, recordSessionBaseline, writeEnvFile, settleTaskResult, spawnAsyncTask, harvestAsyncResults, cleanAsyncDir, hasSignalGatedTasks, injectSignalBlock, forkParallelTask, awaitParallelBatch, killParallelBatch, BUILTIN_EDIT_TOOLS, SIGNAL_PLAN_MARKER }
