...
// Append-only JSONL at <proveItDir>/sessions/<id>/spend.jsonl. An append-only
// ledger rather than a total in session state: detached async workers and
// parallel reviewers write concurrently, so read-modify-write would lose rows.
// Lives in the per-session dir so it is pruned along with the rest of it.

function spendFilePath (sessionId) {
  return path.join(getProveItDir(), 'sessions', sessionId, 'spend.jsonl')
}

function recordReviewerSpend (sessionId, entry) {
  if (!sessionId) return
  try {
    const p = spendFilePath(sessionId)
    ensureDir(path.dirname(p))
    const row = {
      at: Date.now(),
      task: entry.task || null,
      kind: entry.kind || null,
      hookEvent: entry.hookEvent || null,
      model: entry.model || null,
      costUsd: typeof entry.costUsd === 'number' ? entry.costUsd : null,
      numCalls: typeof entry.numCalls === 'number' ? entry.numCalls : null
    }
    if (entry.usage) row.usage = entry.usage
    fs.appendFileSync(p, JSON.stringify(row) + '\n', 'utf8')
  } catch {
    // Best-effort — never fail a review over bookkeeping
  }
}

function readReviewerSpend (sessionId) {
  const empty = { totalUsd: 0, reviews: 0, unknownCostCalls: 0, entries: [] }
  if (!sessionId) return empty

  let content
  try {
    content = fs.readFileSync(spendFilePath(sessionId), 'utf8')
  } catch {
    return empty
  }

  const summary = { totalUsd: 0, reviews: 0, unknownCostCalls: 0, entries: [] }
  for (const line of content.split('\n')) {
    ...
    summary.reviews += 1
    if (typeof row.costUsd === 'number') summary.totalUsd += row.costUsd
    else summary.unknownCostCalls += 1
  }
  return summary
}
