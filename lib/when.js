...
const { isSourceFile, isTestFile, resolveRelativePath } = require('./globs')
...
/**
 * Read the signal a dispatch is evaluating against.
 *
 * The Stop dispatcher snapshots the active signal at entry (`_signalSnapshot`)
 * and may clear the persisted signal partway through its task loop, so every
 * task in that dispatch must see the same value. Callers without a snapshot
 * (unit tests, other entry points) fall back to persisted state.
 */
function activeSignalFor (context) {
  if (context._signalSnapshot !== undefined) return context._signalSnapshot
  return getSignal(context.sessionId)
}

/**
 * Read the phase a dispatch is evaluating against.
 *
 * Consuming a `done` authorization resets the phase mid-loop, so tasks are
 * matched against the phase the dispatch started with. Callers without a
 * snapshot fall back to persisted state.
 */
function activePhaseFor (context) {
  if (context._phaseSnapshot !== undefined) return context._phaseSnapshot
  return getPhase(context.sessionId)
}
...
function evaluateWhenClause (when, context, taskName) {
  ...
  if (when.signal) {
    const activeSignal = activeSignalFor(context)
  ...
  if (when.phase) {
    const currentPhase = activePhaseFor(context)
 ...
 *
 * Also stashes `_signalAuthorized` on the context: true when the task passed
 * and every passing clause needed the active signal to pass. Because array form
 * is OR-of-ANDs, a task with a signal clause can still be eligible by a
 * signal-free route—launching it then must not spend the signal.
 *
...
function evaluateWhen (when, context, taskName) {
  context._signalAuthorized = false
  if (!when) return true

  if (Array.isArray(when)) {
    let anyPassed = false
    let passedWithoutSignal = false
    let passedWithSignal = false
    let lastReason = null
    const allProgress = []
    ...
    for (const clause of when) {
      ...
      if (result === true) {
        anyPassed = true
        if (clause && clause.signal) passedWithSignal = true
        else passedWithoutSignal = true
      } else {
        lastReason = result
      }
    }
    ...
    if (anyPassed) {
      context._signalAuthorized = passedWithSignal && !passedWithoutSignal
      return true
    }
    return lastReason || 'Skipped because no conditions were met'
  }

  const result = evaluateWhenClause(when, context, taskName)
  // Object form is a pure AND, so a passing clause with a signal condition
  // could not have passed without it.
  if (result === true && when.signal) context._signalAuthorized = true
  return result
}
