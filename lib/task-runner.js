...
function buildTaskSnapshot (task, context) {
  ...
  const snapshot = {
    ...
    context: {
      ...
      // Carried explicitly: the worker re-expands templates in its own
      // process, by which time the dispatcher may already have cleared the
      // persisted signal that {{signal_message}} would otherwise read. The
      // phase snapshot travels with it for the same reason ({{phase}}).
      _signalSnapshot: context._signalSnapshot,
      _phaseSnapshot: context._phaseSnapshot
    },
    resultPath
  }
