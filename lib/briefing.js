...
**Signal only at the user's direction.** When you have finished implementing a coherent body of work (a full feature, a complete bug fix, a finished refactor), tell the user it is ready and ask whether to run \`prove_it signal done\`. Run it when they say so — not after every edit, not on your own initiative, and never repeatedly.

### Completion rule

Do not say "done", "finished", "complete", "ready to ship", or similar language about work you have not verified. Signalling is not what makes work done; it is how the user asks for a review of work you believe is already done.

### Accountability rule

After implementing code changes, report your status and ask how the user wants to proceed. Say plainly which of these you are in:

- Blocked on user input or a decision
- An intermediate step in a larger plan
- Tests are currently failing and you're still debugging
- The work is incomplete — here is what's left: [list]
- The work is complete and ready to review

Silence is not acceptable. Report and ask.

### After a reviewer FAIL

Report the findings to the user and stop. Do not auto-remediate, and do not re-signal. The user decides what happens next.`
...
const SIGNAL_DIRECTIVES = {
  done: 'After completing a coherent body of work, ask the user whether to run `prove_it signal done` — the signal is theirs to call, not yours.',
...
function renderBriefing (cfg, runs) {
  ...
  if (signalTypes.size > 0) {
    ...
    lines.push('  - If a reviewer finds significant issues, report them to the user and wait for direction.')
    lines.push('')
  }
